...
class KeyValueFacade:
    ...
    async def get(self, key: str, revision: Optional[int] = None) -> "KeyValue.Entry":
        return await self._key_value.get(key=key, revision=revision)

    async def put(self, key: str, value: "SendableMessage") -> int:
        payload, _ = encode_message(value)
        return await self._key_value.put(key=key, value=payload)

    async def create(self, key: str, value: "SendableMessage") -> int:
        payload, _ = encode_message(value)
        return await self._key_value.create(key=key, value=payload)

    async def update(
        self, key: str, value: "SendableMessage", last: Optional[int] = None
    ) -> int:
        payload, _ = encode_message(value)
        return await self._key_value.update(key=key, value=payload, last=last)

    async def delete(self, key: str, last: Optional[int] = None) -> bool:
        return await self._key_value.delete(key=key, last=last)

    async def purge(self, key: str) -> bool:
        return await self._key_value.purge(key=key)

    async def purge_deletes(self, olderthan: int = 30 * 60) -> bool:
        return await self._key_value.purge_deletes(olderthan=olderthan)

    async def status(self) -> "KeyValue.BucketStatus":
        return await self._key_value.status

    async def watchall(self, **kwargs: Any) -> "KeyValue.KeyWatcher":
        return await self._key_value.watchall(**kwargs)

    async def history(self, key: str) -> list["KeyValue.Entry"]:
        return await self._key_value.history(key=key)
    ...
    def raw_key_value(self) -> "KeyValue":
